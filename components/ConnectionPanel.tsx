...
import { Zap, Cpu, Loader2, Bluetooth, Cable, Globe, AlertCircle, Settings, Info } from 'lucide-react';
            ...
            {isCode2Error && (
              <div className="mb-6 p-6 bg-red-600 rounded-3xl text-white animate-in zoom-in duration-300 shadow-xl">
                ...
                <p className="text-[10px] font-medium leading-relaxed mb-4 opacity-90">
                  Android's Bluetooth stack has reached its limit. No more apps can scan. <br/>
                  <b>Fix:</b> Toggle Bluetooth OFF and ON in your system settings.
                </p>
                <button onClick={handleOpenSettings} className="w-full py-3 bg-white text-red-600 rounded-xl text-[10px] font-orbitron font-black uppercase tracking-widest flex items-center justify-center gap-2 active:scale-95 shadow-lg">
                  <Settings size={14} /> Open System Settings
                </button>
              </div>
            )}

            {!isCode2Error && latestLog && (
              <div className={`mb-8 p-4 rounded-2xl border flex items-start gap-3 ${latestLog.includes('ERROR') ? 'bg-red-50 border-red-100 text-red-600' : 'bg-indigo-50 border-indigo-100 text-indigo-600'}`}>
                <Info size={16} className="mt-0.5" />
                <div className="flex flex-col gap-1">
                  <span className="text-[9px] font-black uppercase opacity-60">Status_Log</span>
                  <p className="text-[10px] font-mono leading-tight">{latestLog}</p>
                </div>
              </div>
            )}
          </div>
