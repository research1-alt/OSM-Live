        ...
        <div className="bg-white rounded-[32px] p-6 md:p-10 shadow-xl border border-slate-200">
          <div className="flex items-center justify-between mb-8">
            <div className="flex flex-col">
              ...
              <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest mt-1">Cross-Platform Bridge</p>
            </div>
          ...
          <div className="grid grid-cols-3 gap-3 mb-6">
            <button 
              ...
              className={`flex flex-col items-center gap-2 p-4 rounded-2xl border transition-all ${hardwareMode === 'pcan' ? 'bg-indigo-600 border-indigo-700 text-white shadow-lg' : 'bg-slate-50 border-slate-100 text-slate-400'}`}
            >
              <Globe size={18}/><span className="text-[8px] font-orbitron font-black uppercase">PCAN</span>
            </button>
            <button 
              onClick={() => onSetHardwareMode('esp32-serial')} 
              className={`flex flex-col items-center gap-2 p-4 rounded-2xl border transition-all ${hardwareMode === 'esp32-serial' ? 'bg-indigo-600 border-indigo-700 text-white shadow-lg' : 'bg-slate-50 border-slate-100 text-slate-400'}`}
            >
              <Cable size={18}/><span className="text-[8px] font-orbitron font-black uppercase">Wired</span>
            </button>
            <button 
              onClick={() => onSetHardwareMode('esp32-bt')} 
              className={`flex flex-col items-center gap-2 p-4 rounded-2xl border transition-all ${hardwareMode === 'esp32-bt' ? 'bg-indigo-600 border-indigo-700 text-white shadow-lg' : 'bg-slate-50 border-slate-100 text-slate-400'}`}
            >
              <Bluetooth size={18}/><span className="text-[8px] font-orbitron font-black uppercase">BLE</span>
            </button>
          </div>
