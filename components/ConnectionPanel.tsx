...
const ConnectionPanel: React.FC<ConnectionPanelProps> = ({ 
  ...
  useEffect(() => {
    ...
    checkSupport();
    const timer = setTimeout(checkSupport, 2000); // Check again after delay
  ...
  const isBluetoothBlocked = hardwareMode === 'esp32-bt' && (!btSupported && !forceBypass);
    ...
    <div className="flex flex-col gap-4 w-full max-w-5xl mx-auto pt-2 animate-in fade-in pb-10 overflow-y-auto max-h-full custom-scrollbar">
      ...
      <div className="bg-indigo-50 border border-indigo-100 p-4 rounded-3xl flex items-center gap-4 shadow-sm">
         ...
         <div className="flex-1">
            ...
            <p className="text-[10px] text-indigo-600 font-medium leading-relaxed">Ensure you are using <span className="font-bold text-indigo-800 underline">Trusted Web Activity (TWA)</span> in Android Studio for Bluetooth support.</p>
         </div>
      </div>
              ...
              <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100 flex flex-col gap-3 animate-in fade-in slide-in-from-top-2">
                 ...
                 <div className="flex flex-col gap-2">
                    <div className="flex items-center justify-between text-[10px] font-bold">
                        ...
                        {(btSupported || forceBypass) ? <ShieldCheck size={14} className="text-emerald-500" /> : <ShieldAlert size={14} className="text-red-500" />}
                    </div>
                    <div className="flex items-center justify-between text-[10px] font-bold">
                 ...
                 {!btSupported && !forceBypass && (
                    <div className="mt-2 p-3 bg-red-50 border border-red-100 rounded-xl">
                        <p className="text-[8px] text-red-600 font-bold uppercase leading-tight mb-2">
                            WebView blocks Bluetooth. Use TWA or Bypass.
                        </p>
                        <button 
